...
async def choose_machine(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("Выберите машину:", reply_markup=kb)
...
async def choose_hour(callback: types.CallbackQuery):
    ...
    if not has_any:
        return await callback.message.edit_text(
            f"На {date} свободных часов не осталось.",
            reply_markup=kb
        )
...
async def finalize(callback: types.CallbackQuery):
    ...
    with get_conn() as conn:
        ...
        row = cur.fetchone()
        if not row:
            return await callback.message.edit_text("Ошибка: машина не найдена.")
...
async def show_help(msg: types.Message):
    help_text = (
        ...
        "Если есть вопросы — пишите в Жалобы в "
    )
